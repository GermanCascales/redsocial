...
class NotificationBar extends Component {
    ...
    protected $listeners = ['notificationBarBtn' => 'loadNotifications'];
    ...
    public function markAsRead($notification_id) {
        ...
        if ($notification->type === 'App\Notifications\CommentPosted') {
            ...
            if (!$post) {
                session()->flash('error_alert', 'El post ya no existe.');
            ...
            if (!$comment) {
                session()->flash('error_alert', 'El comentario ya no existe.');

                redirect()->route('posts.show', $notification->data['post_id']);
            }

            $comments = $post->comments()->orderBy('updated_at', 'desc')->pluck('id');
            $commentIndex = $comments->search($comment->id);
            // dd($comment->getPerPage());

            $commentPage = (int)($commentIndex/$comment->getPerPage()) + 1;

            session()->flash('scrollComment', $comment->id);

            return redirect()->route('posts.show', ['post' => $post->id, 'page' => $commentPage]);
        }
    }

    public function notificationIcon($notificationType) {
