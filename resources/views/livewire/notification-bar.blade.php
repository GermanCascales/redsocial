<div
    x-cloak x-data="{ isOpen: false, blinkCircle: true }"
    x-init="Livewire.on('notificationBarBtn', () => {
                isOpen = true
                setTimeout(() => {
                    blinkCircle = false
                }, 2000)
    ...
    class="z-10 w-full h-full top-0 overflow-y-auto overflow-x-hidden sticky-0 fixed">
    <div x-show="isOpen" x-transition.opacity.duration.400ms class="fixed inset-0 bg-gray-800 bg-opacity-90 transition-opacity"></div>
    <div
        x-show="isOpen" @click.away="isOpen = false"
        class="z-10 right-0 h-full transition overflow-x-hidden"
        ...
        <div class="flex items-center justify-between pb-4">
            <p tabindex="0" class="focus:outline-none text-2xl font-semibold leading-6 text-gray-800">Notificaciones</p>
            <div>
                <button wire:click="markAllAsRead" class="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 rounded-md cursor-pointer mr-5" title="Marcar como leídas">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="#4B5563" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                    </svg>
                </button>
                <button @click="isOpen = false" class="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 rounded-md cursor-pointer" title="Cerrar">
        ...
        <div wire:loading.delay class="w-full">
            <div class="flex justify-center items-center">
                <svg class="animate-spin w-8 h-8 text-blue" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
                <div class="ml-2">Cargando...</div>
            </div>
        </div>
        ...
        @if ($notifications->isEmpty())
            <div wire:loading class="w-full">
                @foreach (range(1,5) as $item)
                    <div class="animate-pulse w-full p-3 mt-4 bg-white rounded flex shadow flex flex-shrink-0">
                        <div class="focus:outline-none w-8 h-8 border rounded-full bg-gray-200 border-gray-200 flex items-center justify-center">
                        </div>
                        <div class="pl-3 w-full">
                            <div class="flex items-center justify-between w-full bg-gray-200 h-10">
                            </div>
                        </div>
                    </div>    
                @endforeach
            ...
            <div wire:loading.remove class="mx-auto">
                <div class="text-gray text-center font-bold mt-6">No hay notificaciones</div>
            </div>
        @else
            @foreach ($notifications as $notification)
            <div wire:key="{{ $notification->id }}" class="w-full p-3 mt-4 bg-white rounded flex shadow flex flex-shrink-0 @if($notification->read_at == null) border border-blue @endif">
                ...
                <div class="flex flex-col justify-between pl-2">
                    @if($notification->read_at == null)
                        <span class="flex relative h-3 w-3">
                            <span x-show="blinkCircle" class="animate-ping absolute inline-flex h-full w-full rounded-full bg-sky-400 opacity-75"></span> 
            ...
            <div class="w-full p-3 mt-4 bg-white rounded shadow flex flex-shrink-0">
                ...
                <div class="pl-3 w-full">
                <div class="flex items-center justify-between w-full">
                    <p tabindex="0" class="focus:outline-none text-sm leading-none"><span class="font-semibold text-indigo-700">Sash</span> added you to the group: <span class="font-semibold text-indigo-700">UX Designers</span></p>
                    <div tabindex="0" aria-label="close icon" role="button" class="focus:outline-none cursor-pointer">
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M10.5 3.5L3.5 10.5" stroke="#4B5563" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M3.5 3.5L10.5 10.5" stroke="#4B5563" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round" />
            ...
            <div class="w-full p-3 mt-4 bg-white rounded flex">
                ...
                <div class="pl-3">
                <p tabindex="0" class="focus:outline-none text-sm leading-none"><span class="font-semibold text-indigo-700">Sarah</span> posted in the thread: <span class="font-semibold text-indigo-700">Update gone wrong</span></p>
                <p tabindex="0" class="focus:outline-none text-xs leading-3 pt-1 text-gray-500">2 hours ago</p>
                </div>
            </div>
            ...
            <div class="w-full p-3 mt-4 bg-white rounded flex">
                ...
                <div class="pl-3">
                <p tabindex="0" class="focus:outline-none text-sm leading-none">Shipmet delayed for order<span class="font-semibold text-indigo-700"> #25551</span></p>
                <p tabindex="0" class="focus:outline-none text-xs leading-3 pt-1 text-gray-500">2 hours ago</p>
                </div>
            </div>
            ...
            <div class="w-full p-3 mt-4 bg-white rounded flex">
                ...
                <div class="pl-3">
                <p tabindex="0" class="focus:outline-none text-sm leading-none"><span class="font-semibold text-indigo-700">James Doe</span> favourited an <span class="font-semibold text-indigo-700">item</span></p>
                <p tabindex="0" class="focus:outline-none text-xs leading-3 pt-1 text-gray-500">2 hours ago</p>
                </div>
            </div>
