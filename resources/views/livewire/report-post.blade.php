<div
    ...
    x-init="Livewire.on('postReported', () => {
                isOpen = false
            })"
    x-show="isOpen"
    @keydown.escape.window="isOpen = false"
    @custom-show-report.window="isOpen = true"
        ...
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <div class="sm:flex sm:items-start">
            ...
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Marcar post como inapropiado</h3>
              <div class="mt-2">
                <p class="text-sm text-gray-500">¿Estás seguro de que quieres marcar este post como inapropiado?</p>
              </div>
            </div>
          </div>
        </div>
        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
          <button wire:click="reportPost" type="button" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-amber-500 text-base font-medium text-white hover:bg-amber-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">Confirmar</button>
